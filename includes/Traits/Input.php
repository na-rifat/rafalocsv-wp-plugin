...
namespace Rafalocsv\Traits;

use Rafalocsv\Rafalocsv;
use Rafalocsv\Schema\Schema;

trait Input {
    public $__        = Rafalocsv::domain;
    public $domain    = Rafalocsv::domain;
    public $supported = [
        'text',
        'email',
        'url',
    ...
    public static function create_settings_form( Array $args = [] ) {
        ...
        return sprintf( '<form data-name="%s" class="rafalocsv-schema-form %s" method="POST", action="%s">%s</form>',
            $args['settings_key'],
            implode( ' ', $args['class'] ),
            '',
            $fields );
    ...
    public static function submit( Array $args = [], bool $admin = true ) {
        $self     = new self();
        $defaults = [
            'id'    => 'rafalocsv-submit-button',
            'label' => __( 'Submit', $self->__ ),
            'class' => [],
        ];
        $args = wp_parse_args( $args, $defaults );

        $button = sprintf( '<div id="%s" class="rafalocsv-submit-button button button-large %s"><div class="loader"></div>%s</div>', $args['id'], implode( ' ', $args['class'] ), $args['label'] );
