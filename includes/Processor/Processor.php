...
namespace Rafalocsv\Processor;

use Rafalocsv\Rafalocsv;
...
class Processor {
    ...
    public static function generate_logs( $inserted, $skipped ) {
        ob_start();
        new \Rafalocsv\Admin\Uploadlog( $inserted );
        $inserted = ob_get_clean();

        ob_start();
        new \Rafalocsv\Admin\Skipped( $skipped );
    ...
    public static function sanitize_page_content( $content ) {
        if ( preg_match( '/^\{?[A-Za-z0-9]{8}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{12}\}?$/', $content ) ) {
            ...
            $api_key = \Rafalocsv\Schema\Schema::get_settings( 'header' )['seo-textwriter-api-key']['value'];
            ...
            if ( $result == 'Error: not authorized' ||
                $result == "Error: The parameter '4f64a756-05ca-482b-a4cc-671583e117d4f' is not a valid guid" ) {
                return false;
            }
    ...
    public static function collect_meta_fields( $data, $image_urls, $modified_time ) {
        $fields = \Rafalocsv\Schema\Schema::get( 'meta_fields' );
        $result = '';
    ...
    public static function create_page( $data, $image_files ) {
        ...
        if ( $content == false ) {
            self::json_status(
                [
                    'success' => false,
                    'msg'     => __( 'An error occured while gettting content from the API.', 'rafalocsv' ),
                ]
            );
            exit;
        }
        ...
        wp_update_post(
            [
                ...
                'post_content' => self::prepare_page_content_after( [
                    ...
                    'SiteUrl'       => site_url(),
                    'ImageFolder'   => 'rafalocsv',
                    'UpdateDate'    => sprintf( '<span data-mergefield="UpdateDate">%s</span>', $modified_date ),
                    'PublishDate'   => sprintf( '<span data-mergefield="PublishDate">%s</span>', $publish_date ),
                ], $post_content ),
                'meta_input'   => [
                    'rafalocsv-meta' => self::collect_meta_fields(
                        $data,
                        $image_urls,
                        $modified_date
                    ),
    ...
    public static function upload_feature_image( $post_id, $img ) {
        ...
        $base_dir   = $upload_dir['basedir'] . '/rafalocsv';
    ...
    public static function create_interlink_col( $box_title, $interlinks ) {
        ...
        $el = sprintf( '<div class="interlink-col col-md-4"><h3>%s</h3><ul>%s</ul></div>', $box_title, $list );
    ...
    public static function create_breadcrumb( $data ) {
        ...
        if ( isset( $data['ParentPage'] ) && get_page_by_title( $data['ParentPage'] ) !== null ) {
            ...
            $parent     = sprintf( '<a href="%s"><strong>%s</strong></a> >> ', site_url( '/' . $parent_obj->post_name ), $parent_obj->post_title );
        }
